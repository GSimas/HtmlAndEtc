                        ...
                        <ul>
							...
							{ ?>
							    ...
							    <?php if (has_post_thumbnail( $post->ID ) ):
										$cooper_image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'qu-blog' );?>
                                    <li>
									<?php if (( get_post_meta($post->ID,'rnr_sec-menu-title',true))){?>
									<a class="scroll-link fbgs" href="#<?php echo balanceTags($post->post_name);?>" data-bgscr="<?php echo esc_url($cooper_image[0]);?>" data-bgtex="<?php echo (get_post_meta($post->ID,'rnr_sec-menu-title',true)); ?>">
									<?php } else {?>
									<a class="scroll-link fbgs" href="#<?php echo balanceTags($post->post_name);?>" data-bgscr="<?php echo esc_url($cooper_image[0]);?>" data-bgtex="<?php the_title();?>"><?php };?>
