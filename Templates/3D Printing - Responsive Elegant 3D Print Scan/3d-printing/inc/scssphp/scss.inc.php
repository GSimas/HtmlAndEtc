...
class scss_formatter {
	...
	protected function block($block) {
		...
		if (!empty($block->lines)) {
			...
			echo do_shortcode($inner) . implode($glue, $block->lines);
			if (!empty($block->children)) {
				echo do_shortcode($this->break);
			}
		}
		...
		if (!empty($block->selectors)) {
			$this->indentLevel--;
			if (empty($block->children)) echo do_shortcode($this->break);
			echo do_shortcode($pre . $this->close . $this->break);
		}
	}
...
class scss_server {
	...
	public function serve($salt = '') {
		...
		if ($input = $this->findInput()) {
            ...
            WP_Filesystem();
            global $wp_filesystem;
			echo do_shortcode($wp_filesystem->get_contents($output));
